#!/usr/bin/env node
/**
 * Append game/validation/sunken-canyon/harness-blocker-fixes.md to findings.md
...
const OUT_DIR = path.join(GAME_ROOT, 'validation', 'sunken-canyon');
...
if (!fs.existsSync(FINDINGS_PATH)) {
	console.error('append-sunken-canyon-harness-fixes: missing findings.md');
	process.exit(1);
}
if (!fs.existsSync(FIXES_PATH)) {
	console.error('append-sunken-canyon-harness-fixes: missing harness-blocker-fixes.md');
	process.exit(1);
}
