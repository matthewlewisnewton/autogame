...
Players must register or log in through the auth overlay before connecting. The client calls `/api/register` or `/api/login`, which set an httpOnly `ag_session` cookie (`setSessionCookie` in `game/server/auth.js`), then opens a Socket.IO connection via `createSocket()` in `game/client/main.js` with `withCredentials` so the cookie is sent on the WebSocket handshake. Before the connection handler runs, the server's `io.use()` middleware in `game/server/index.js` reads `cookies.ag_session`, validates it via `getSession()`, and rejects missing or invalid sessions with `connect_error` (`Missing or invalid session` / `Invalid or expired session`); the client re-shows login.
