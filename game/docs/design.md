...
**Stage variants:** `spire-ascent` is a vertical tower layout (3–5 tiers stacked along +Z with ramp passages). Quest defs set `layoutStage: 'spire-ascent'` (e.g. `spire_ascent` in `quests.js`); `applyLayoutForQuest` passes it to `generateLayout`. Debug shortcut: `?debugScenario=spire-ramp-passage`.
