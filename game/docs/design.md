...
## Overview
This game is a 3D multiplayer action-RPG that combines elements from *Phantasy Star Online Episodes I&II* and *Lost Kingdoms 1 & 2*. Players will gather in a lobby, join lobbies, and tackle semi-procedurally generated dungeons to defeat enemies and gather loot. Combat revolves around an active "deck" of cards rather than basic attacks.

## Core Loop
1. **Lobby browser**: After login, players see a list of open lobbies (id, name, hostId, gamePhase, selectedQuestId, playerCount, players). The `gamePhase` field indicates whether a lobby is waiting (`'lobby'`) or actively in a run (`'playing'`); `selectedQuestId` shows which quest the lobby has chosen. They create a new lobby or join an existing one. Lobbies mid-run support **drop-in** rejoin.
2. **Lobby**: Once in a lobby, players manage decks, select quests, trade, and ready up. See [lobbies.md](./lobbies.md) for socket events, server architecture, and testing.
3. **Dungeon**: When all lobby members are ready, players zone into a modularly generated level where they navigate rooms, defeat AI enemies, and complete objectives.
...
### Floor Geometry
Dungeon rooms and passages may have sloped floors (ramps) with varying elevation across their vertices. The walkable surface height at any `(x, z)` coordinate is determined by `sampleFloorY()`, which interpolates across the room's floor corner positions. Each room carries a `floorCorners: { yNW, yNE, ySE, ySW }` object specifying the Y height at each corner; corners are labelled NW/NE/SE/SW relative to room center (NW = top-left, counter-clockwise). Player movement on slopes — adjusting `player.y` to follow the floor surface — uses `sampleFloorY()` for height interpolation. Floor sampling logic lives in `shared/floorSampling.esm.js`; see `shared/floorSampling.js` for the CJS eval-bridge.

## Run Suspend / Resume
Telepipe is a mid-run evacuation spell that lets a lobby suspend a dungeon without losing the current run state.
...
- Each player enters the portal individually. Extracted players return to the lobby overlay, cannot use dungeon actions, and are ignored by combat simulation while remaining lobby members continue playing.
...
- When the lobby Deploys again from the suspended lobby, the checkpoint is restored instead of generating a fresh layout, and the dungeon resumes with the preserved portal/enemy/loot state.
- Players can abandon a suspended run from the lobby, which clears the checkpoint and returns the lobby to normal lobby flow.
