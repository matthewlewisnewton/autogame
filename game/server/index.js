...
function startServer(port) {
  ...
  socket.on('useKeyItem', (data) => {
    ...
    if (keyItemId === 'smoke_bomb') {
      // --- smoke_bomb: drop a fog zone fixed at the caster's position. Enemies
      // lose track of players inside it; the detection rule is handled in
      // sub-ticket 02 and the client VFX in sub-ticket 03. Here we only set the
      // transient zone state, burn cooldown, and broadcast. ---
