...
function applyDebugScenario(socket, name) {
  ...
  return withLobbyContext(lobby, () => {
    ...
    if (name === 'summon-low-mana') {
      ...
      // Load the spire-ascent tower layout for render / collision QA (profile
      // generateLayout(seed, 'spire-ascent')). Same layout reachable when a quest
      // uses layoutProfile 'spire-ascent'; this scenario is a dev shortcut only.
      player.hp = MAX_HP;
      player.magicStones = MAX_MAGIC_STONES;
      const seed = state.layoutSeed || 42;
      state.layoutSeed = seed;
      ...
      // spire_ascent normally; shortcut for QA (enemies, layout, tier-0 spawn).
      player.hp = MAX_HP;
      player.magicStones = MAX_MAGIC_STONES;
