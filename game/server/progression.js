...
const EVOLUTION_TRANSFORMS = require('../shared/evolutionTransforms.json');
const CARD_SELL_VALUES = require('../shared/cardSellValues.json');
...
// Server-side card definitions: the full shared stat set (id/name/type/charges
// plus client-relevant fields like damage/effect/specialEffect/magicStoneCost)
// comes from the shared ../shared/cardDefs.json; each entry merges in only the
// server-only fields (minion/breath/attack/dot/shield stats, cooldowns, and
// Math.*/TICK_RATE-derived values) that the client never needs.
const CARD_DEFS = {
  iron_sword: { ...CARD_IDENTITY.iron_sword },
  flame_blade: { ...CARD_IDENTITY.flame_blade },
  battle_familiar: { ...CARD_IDENTITY.battle_familiar },
  dungeon_drake: {
...
// EVOLUTION_TRANSFORMS and CARD_SELL_VALUES are now sourced from the shared
// JSON single sources (required at the top of this module). CARD_SELL_VALUES is
// the reconciled superset of the formerly drifted server/client copies.
