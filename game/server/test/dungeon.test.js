...
describe("generateLayout(seed, 'spire-ascent')", () => {
  ...
  function tierCenterY(room) {
    return sampleFloorY({ rooms: [room], passages: [] }, room.x, room.z);
  }
  ...
  function spireFullyReachable(layout) {
    const colliders = buildWallColliders(layout);
    const aabbs = computeWalkableAABBs(layout);
    return countReachableRooms(layout, aabbs, colliders) === layout.rooms.length;
  }
  ...
  it('returns profile spire-ascent with empty passages', () => {
    ...
    expect(roomsByBand(layout, 'ramp').length).toBe(
      roomsByBand(layout, 'tier').length - 1
    );
  });
  ...
  it('each tier is flat with width/depth in ~12–15 range and monotonic tier Y', () => {
  ...
  it('tierIndex runs 0..n-1 from bottom to top', () => {
    const layout = generateLayout(42, 'spire-ascent');
    const tiers = tiersSorted(layout);
  ...
  it('each ramp has band ramp, slope ≥ 0.2, and non-uniform corners', () => {
  ...
  it('total Y gain from start tier to top tier is ≥ 10', () => {
    for (const seed of [1, 42, 123, 777, 9999]) {
      const layout = generateLayout(seed, 'spire-ascent');
      const tiers = tiersSorted(layout);
      const start = tiers[0];
      ...
      const yTop = sampleFloorY(layout, top.x, top.z);
      expect(yTop - yStart).toBeGreaterThanOrEqual(10);
    }
  });

  it('assigns explicit roles: bottom=start, top=treasure, ramps=connector spawnWeight 0', () => {
    const layout = generateLayout(42, 'spire-ascent');
    const tiers = tiersSorted(layout);
  ...
  it('every tier and ramp has perimeter side walls', () => {
    const layout = generateLayout(42, 'spire-ascent');
    for (const tier of roomsByBand(layout, 'tier')) {
      expect(tier.walls.length).toBeGreaterThanOrEqual(3);
      expect(tier.walls.some(w => w.axis === 'z')).toBe(true);
      expect(tier.walls.some(w => w.axis === 'x')).toBe(true);
    }
    for (const ramp of roomsByBand(layout, 'ramp')) {
      expect(ramp.walls.length).toBeGreaterThanOrEqual(2);
      expect(ramp.walls.every(w => w.axis === 'z')).toBe(true);
    }
  });

  it('full foot reachability from bottom spawn to all rooms', () => {
    for (const seed of [1, 42, 123, 777, 2024, 9999]) {
      const layout = generateLayout(seed, 'spire-ascent');
      expect(spireFullyReachable(layout)).toBe(true);
    }
  });

  it('spawn can reach top-tier center via walkable AABBs', () => {
    function canReachPoint(fromX, fromZ, toX, toZ, aabbs, colliders) {
      const tolerance = 1.5;
      const seen = new Set();
      const key = (x, z) => `${Math.round(x * 10)},${Math.round(z * 10)}`;
