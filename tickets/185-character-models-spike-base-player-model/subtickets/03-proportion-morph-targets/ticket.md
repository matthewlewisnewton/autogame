...
- A small automated check exists that loads `player.glb` and asserts all six morph
  names are present (test under `game/client/test/`; follow existing vitest patterns).
  The test is included in `pnpm test:quick`.
- Normalization from sub-ticket 02 is preserved (feet `y=0`, −Z forward, ~1.8u height).
- Still **no** runtime avatar swap — do not modify `renderer.js`, `models.js` registry
  paths, server cosmetic validation, or customization UI.
...
- **New** test file, e.g. `game/client/test/playerModelMorphs.test.js` (name flexible):
  peek the glTF JSON chunk (or use `@gltf-transform/core` / `GLTFLoader`) to verify
  morph target names without starting the game loop.
- **Edit** `game/vitest.config.js` or `game/client/package.json` only if needed so the
  new test is picked up by the existing vitest config (`client/test/**`).
- Optional dev-only helper under `game/client/scripts/` (e.g. `add-player-proportion-morphs.mjs`)
  to regenerate morphs from Blender exports — not required if the committed `.glb` passes
  the test.
- Do **not** change server `cosmetic` validation, customization UI, or `createPlayerAvatar`.
