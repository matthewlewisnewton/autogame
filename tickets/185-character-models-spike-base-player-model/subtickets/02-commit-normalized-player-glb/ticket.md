...
- **Replace or add** `game/client/public/models/player.glb` — export from Blender (or copy from the
...
- Optional dev-only authoring helpers under `game/client/scripts/` (e.g. Blender export
  or Node normalizer using `@gltf-transform/core`) and `game/client/package.json` dep
  additions — not required for acceptance if the committed `.glb` already meets the contract.
