...
- Tier 2 deploy spawns the quest's `enemyCount` with band-aware placement (`pickSunkenCanyonEnemySpawn`: at least one enemy on plateau, majority on canyon); at least one spawned enemy has a non-null `variant` under a fixed seed test.
- Tier 1 `canyon_descent` behavior is unchanged: default layout mode, near-zero variant tags on the same seed batch, and existing `sunken_canyon_spawn.test.js` assertions still pass.
...
- Optional debug shortcut `canyon-descent-tier-2` (behind `ALLOW_DEBUG_SCENARIOS=1`) sets `selectedQuestId` / `selectedQuestTier` and applies the Tier-2 layout **before** `enterPlayingPhase()` so `run.questTier` and variant rolls match normal deployment (follow the `arena-trials-tier-2` pattern from ticket 254).
...
- **`game/server/debugScenarios.js`**
  - Add `canyon-descent-tier-2` scenario: unlock Tier 2 if needed, set `selectedQuestId = 'canyon_descent'` and `selectedQuestTier = 2`, call `applyLayoutForQuest(state, 'canyon_descent', 2)`, position player on plateau spawn, then `enterPlayingPhase` / `spawnEnemies` in the correct order (early branch before generic `enterPlayingPhase` if required).
  - Register the scenario name in `game/server/index.js` debug allowlists.
- **`game/server/test/canyon_descent_tier2.test.js`** (new, modeled on `arena_trials_tier2.test.js` / `spire_ascent_tier2.test.js`)
  ...
  - Deploy spawn: enemy count, plateau/canyon band split, Tier-2 variant tagging, Tier-1 null variants.
  - Socket test: Tier 1 victory unlocks Tier 2 for `canyon_descent`.
- **`game/server/test/quests.test.js`** — Extend variant catalog expectations for `canyon_descent` Tier 2 (`layoutMode`, `layoutProfile`, unlock metadata); update `listQuestVariants` tier-2 count from 4 to 5.
- **`game/server/test/sunken_canyon_spawn.test.js`** — Extend or add Tier-2 deploy cases if rigid-layout fixtures need explicit coverage beyond the new tier2 file.
- **`game/server/test/debug-scenarios.test.js`** — Assert `canyon-descent-tier-2` sets `run.questTier === 2` and uses rigid layout options when `ALLOW_DEBUG_SCENARIOS=1`.
- **`game/server/test/quest_tier_gating.test.js`** — Ensure locked Tier 2 selection for `canyon_descent` remains covered once the tier exists.
