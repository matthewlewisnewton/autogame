...
- Tier 2 deploy spawns the quest's `enemyCount` (6) with canyon identity preserved: at least one enemy on plateau, majority on canyon band, none on ramp connectors; at least one spawned enemy has a non-null `variant` under a fixed seed test.
- Tier 1 `canyon_descent` behavior is unchanged: default layout mode, near-zero variant tags on the same seed batch, and existing `sunken_canyon_spawn.test.js` band logic still passes.
...
- **`game/server/debugScenarios.js`**
  ...
  - Register the scenario name in `DEBUG_SCENARIOS` and `DEBUG_SCENARIOS_WITHOUT_DEFAULT_SPAWN` in **`game/server/index.js`** when needed to avoid incorrect first spawns.
- **`game/server/test/canyon_descent_tier2.test.js`** (new, modeled on `arena_trials_tier2.test.js` / `spire_ascent_tier2.test.js`)
  ...
  - Rigid sunken-canyon layout stable across seeds for Tier 2; Tier 1 default layout still varies ramp count or cover across seeds.
  - Deploy spawn: enemy count, plateau/canyon band distribution, Tier-2 variant tagging, Tier-1 null variants.
...
- **`game/server/test/sunken_canyon_spawn.test.js`** — Extend Tier-2 deploy cases if band/spawn assertions need a rigid-layout fixture and `getLayoutGenerationOptions('canyon_descent', 2)`.
...
- Reuse sub-ticket 01 rigid sunken-canyon generator and ticket 254's `resolveVariantRollTier` / `getLayoutGenerationOptions`; do not reimplement those mechanisms.
