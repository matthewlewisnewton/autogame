...
implementation is in the working tree right now; read the actual files named by
the ticket, which are the source of truth. To see exactly what changed this
iteration, run `git diff HEAD -- . ':!tickets'` yourself (the change is not yet
committed).
...
1. Confirm every item in the sub-ticket's `## Acceptance Criteria` is satisfied
   ...
   you are judging is the END STATE — read the relevant source files to confirm,
   not just the diff. A criterion that is already satisfied by pre-existing code
   still counts as met. In particular, if the ticket is
...
2. Check the logs and metrics confirm the game still starts and runs. For
   harness/tooling tickets, also inspect the relevant harness logs/tests or
   script behavior named by the ticket. When local-check artifacts are present,
   they should show `rc: 0`; a nonzero local-check status is already a hard
   harness failure and must not be accepted. IGNORE benign environment/library
   noise (THREE.js deprecation warnings, headless WebGL "context lost/restored"
   messages, Vite `ws proxy` / `EPIPE` lines on socket close) — only a genuine
   error from the game's own code counts.
