...
class TestCaptureRunStopGamePorts:
    """Regression: capture_run must pass ports to stop_game so parallel workers
    do not blanket-kill sibling servers."""

    def test_capture_run_passes_ports_to_stop_game(self, tmp_path, monkeypatch):
        stop_calls = []
        monkeypatch.setattr(cr_mod, "start_game", lambda d, p: None)
        monkeypatch.setattr(cr_mod, "wait_for_game", lambda p, timeout_s=45: True)
        ...
        monkeypatch.setattr(
            cr_mod,
            "stop_game",
            lambda ports=None: stop_calls.append(ports),
        )
        ports = PortAllocation(game_server=3001, vite=5174)
        ok = cr_mod.capture_run(tmp_path, game_url="http://localhost:5173", ports=ports)
        assert ok is True
        assert len(stop_calls) == 1
        assert stop_calls[0] is ports
